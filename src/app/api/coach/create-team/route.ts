...
export async function POST(request: NextRequest) {
  try {
    ...
    const { team_name, coach_id } = body as { team_name: string; coach_id: string };

    if (!team_name || !coach_id) {
      return NextResponse.json(
        { error: "Missing required fields: team_name and coach_id" },
        { status: 400 }
      );
    }
    ...
    if (!newTeam) {
      return NextResponse.json(
        { error: "Team creation failed, no data returned" },
        { status: 500 }
      );
    }
